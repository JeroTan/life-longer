...
const biolearnCsvPath = path.join(
  __dirname,
  "../../../.raw-data/biolearn-library/GSE41169_metadata.csv",
);
const agingBiomarkersJsonPath = path.join(
  __dirname,
  "../../../.raw-data/Human Aging and longevity landscape/Aging_Biomarkers.json",
);
const drugageCsvPath = path.join(
  __dirname,
  "../../../.raw-data/DrugAge The Database of Ageing-related Drugs/drugage.csv",
);
...
async function main() {
  ...
  for (let i = 0; i < bList.length; i += 500) {
    ...
    const values = batch
      .map((b) => `('${escapeSql(b.name)}', '${escapeSql(b.category)}')`)
      .join(",\n  ");
  ...
  for (let i = 0; i < iList.length; i += 500) {
    ...
    const values = batch
      .map((b) => `('${escapeSql(b.name)}', '${escapeSql(b.type)}', 'Pending', 'Unknown')`)
      .join(",\n  ");
